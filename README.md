...
2. 画面左下の `><` アイコンをクリック → **「Reopen in Container」** を選択
3. 初回はコンテナのビルドに数分かかります。完了すると自動的にすべてのサービスが起動します
...
**できたところまでで構いません。** GitHubリポジトリにコードをプッシュして提出してください。
提出先のURLは、応募フォームまたは担当者へのメールにてお知らせください。
